...
# Never blocks, never changes the tool input. Off switches: `mda nudge off` (per root, config.toml)
# and `mda nudge off --global` (writes DATA/nudge.off).
...
# Only nudge when this project actually has an index, and not when `mda nudge off` was run
# for this root (`nudge = false` in its config.toml).
project="${CLAUDE_PROJECT_DIR:-$PWD}"
[ -f "$project/.markdownattractor/index.sqlite" ] || exit 0
grep -Eq '^[[:space:]]*nudge[[:space:]]*=[[:space:]]*false' "$project/.markdownattractor/config.toml" 2>/dev/null && exit 0
