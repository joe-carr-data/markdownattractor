...
4. Cards (§4): 4a committed cards (regeneration, exact): assert their hashes against `evals/results/docsqa/FROZEN.md`, rebuild a clean copy of each checkout from them, compare every row and question; or simply run `scripts/eval/preflight.sh development <project>` and compare its report with the committed one; 4b regeneration through the user's own Claude Code login only when asked (hours of wall-clock), published as an independent rerun.
