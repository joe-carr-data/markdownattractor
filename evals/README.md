...
Rows: `lexical (raw only)`; with cards attached (`--cards`, recorded per corpus and `mda` version under `evals/results/docsqa/`) also `lexical (cards + raw)` and, when the embedding model is on disk, `hybrid`. `--out` writes `coverage.json`, `split.json` (every question's split, so the sealed holdout is visible and never touched) and `results.json` (per-question ranks and the first ten distinct pages, from which every metric regenerates without a store). Excluded and stated: questions whose reference evidence is image-derived text (`image_text_evidence_used`), and questions with a label we cannot map or did not index. `requires_multimodal_judgment` is counted for the answer-quality axis. `--split holdout` is refused by convention until 1.0 (it prints a warning; rule 0.2).
...
**Competitor arms (execution plan §2.1, M2).** `scripts/eval/arms/qmd.sh build|drive|record`, `scripts/eval/arms/graphify.sh build|record|drive` (`GRAPHIFY_MODEL=haiku` for the Haiku-built configuration) and `scripts/eval/bm25-files.sh build|drive` build each arm once per freeze and write its rows for `--arm-output`; `scripts/eval/mcp-time.sh <arm> <project> <out>` measures latency through the arm's MCP server with `crates/mda-cli/examples/mcp_time.rs` (cold first call, warm after). Arm records (version, install, effective configuration, build times and usage, coverage, artifact hashes) live under `results/docsqa/arms/`, archived graphs under `results/docsqa/arms/graphs/`, and each arm's rows, scorer output, request and latencies under `results/docsqa/<project>/arms/`. Every script that spawns a model session or an arm's server unsets provider keys first. The runbook's §4c is the reproduction recipe.

**Tuning (execution plan §3, development numbers only).** `scripts/eval/tune.sh baseline` scores the hybrid row on the four dev splits with the current defaults; `tune.sh trial <name> <key=value>…` applies one pre-declared change to the stores' `config.toml` (the five keys of `docs/design/search.md`; `FETCH=<n>` for the adapter's fetch depth), scores it, appends the row to `results/docsqa/TUNING.md` with the objective (mean success@5), the guardrail and the greedy decision, and restores the config; `tune.sh keep <name>` makes a kept candidate the new base. Test and holdout are never scored by it. `tune.sh explore post-stop-<cN> <key=value>` runs a candidate after the stop rule fired, against the unchanged pre-tuning configuration, information only (plan §3, 2026-09-24 amendment): its row carries paired wins/losses and a 95% paired-bootstrap interval from `mda eval --compare <label> <baseline results.json> <candidate results.json>…` (5,000 draws, seed 20260922) and a decision `keep` refuses.
...
**T1 (published, test split).** `evals/results/docsqa/T1/`: `FROZEN.md` (final freeze), per project `results.json`, `arms/<arm>.{jsonl,results.json,driver.jsonl,request*.json,times*.jsonl,manifest.json}`, `latency/<arm>.jsonl`, `pool/` (sample, both judges' judgments, pooled labels, column with intervals), `preflight/T1-<project>.json`. `scripts/eval/t1.sh` runs, archives and renders it; runbook §7.1 reproduces it.

**T2 harness (M5).** `mda eval --export-questions` (the seeded, stratified sample with reference answers), `scripts/eval/t2.sh run|status|grade|analysis` (manifest first, resumable rows, exact per-turn tokens, grounding against whole cited pages), `mda eval --analysis` (plan §2.5: failures as 0, paired bootstrap, the three gates, savings only where they pass), `scripts/eval/panel.sh regrade|cards` (Fable + Astra), `scripts/eval/tests/failure-matrix.sh` (run by `make check`). Runbook §5c.
