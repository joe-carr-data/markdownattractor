...
**Tuning (execution plan §3, development numbers only).** `scripts/eval/tune.sh baseline` scores the hybrid row on the four dev splits with the current defaults; `tune.sh trial <name> <key=value>…` applies one pre-declared change to the stores' `config.toml` (the five keys of `docs/design/search.md`; `FETCH=<n>` for the adapter's fetch depth), scores it, appends the row to `results/docsqa/TUNING.md` with the objective (mean success@5), the guardrail and the greedy decision, and restores the config; `tune.sh keep <name>` makes a kept candidate the new base. Test and holdout are never scored by it.
