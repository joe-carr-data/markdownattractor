...
The greedy loop of the execution plan §3 runs with `scripts/eval/tune.sh` against the four carded stores and logs every trial in `evals/results/docsqa/TUNING.md` (objective, guardrail, decision, code SHA, latency, elapsed). To re-run one trial: `scripts/eval/tune.sh baseline`, then `scripts/eval/tune.sh trial <name> <key=value>` with the key from the table's "config diff" column (`FETCH=<n>` in the environment for the fetch-depth candidate); an `embedding_text` trial re-embeds every card under a new model id (local, ≈ 1 h for the four corpora). The rows you get must equal the logged ones (same stores, same binary at the logged SHA): regeneration, exact. The winner's settings are the defaults of the frozen binary at M4 and appear in `FROZEN.md`'s search configuration line.
