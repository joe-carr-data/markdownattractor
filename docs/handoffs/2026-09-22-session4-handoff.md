...
- **Carding rate measured** on Tailwind with the `claude-cli` backend (policy acknowledged on that checkout's `config.toml`): 200 sections in 182 s, 0 failures, $0.85 list-price equivalent → all four corpora ≈ 10 h and ≈ $175 equivalent. Above the plan's $60 figure: **ask the owner before running**. Tailwind's index now holds 216 cards + vectors (1,116 sections pending); the other three checkouts have raw indexes only.
