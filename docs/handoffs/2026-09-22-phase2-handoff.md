...
| Secrets | `~/.config/markdownattractor/env` (mode 600) holds `ANTHROPIC_API_KEY` and `ANTHROPIC_WORKSPACE_ID=wrkspc_01VmxUCCLAiNK1cJFXwYRbBr`. Load with `set -a; source ~/.config/markdownattractor/env; set +a`. Never print or commit. **Owner decision 2026-09-22: no rotation needed.** The permission classifier blocks reading `.env`-style files; ask the owner rather than searching. |
