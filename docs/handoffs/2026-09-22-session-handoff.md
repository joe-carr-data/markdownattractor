...
- **Secrets on this machine**: `~/.config/markdownattractor/env` (mode 600) holds `ANTHROPIC_API_KEY` and `ANTHROPIC_WORKSPACE_ID=wrkspc_01VmxUCCLAiNK1cJFXwYRbBr` (the other id the user gave, `wrkspc_01CvEdornHRcrCSWDm9xh3op`, returns "workspace not found" for this key). Load with `set -a; source ~/.config/markdownattractor/env; set +a`. **Never print or commit the value.** The key passed through the chat transcript once; the user should rotate it. Memory note: `~/.claude/projects/-Users-jcarr-markdownattractor/memory/anthropic-api-key-location.md`.
