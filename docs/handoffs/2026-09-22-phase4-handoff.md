...
| Open branch | `docs/phase4-wrap`: release workflow target fix, aha lines, this handoff, STATUS. Open a PR for it, merge when CI is green, then re-run the release dry run (`gh workflow run release.yml --ref main`, read the job list with `gh run view <id>`). |
| Release workflow dry run | run `35707331315`: `version agrees` ok, `build (darwin-x64)` failed (`can't find crate for core`: target not on the pinned toolchain), the other four builds were still running at handoff time; read their conclusions before assuming they pass. `publish` runs only when every build succeeds. |
...
1. Open a PR for `docs/phase4-wrap`, merge when CI is green, then `gh workflow run release.yml --ref main` and read the job list. Fix what it shows (likely candidates: the Intel cross build of `aws-lc-sys`/`ort` absent so should be fine; Windows `tar` packing; `ubuntu-22.04-arm` runner name; the `bootstrap.sh` gate in `publish`).
