...
- GitHub provides native runners for every target we ship: `macos-latest` (arm64, also cross-compiles the Intel lexical-only build with Apple's toolchain), `ubuntu-22.04` (x64, oldest glibc we care about), `ubuntu-22.04-arm` (arm64), `windows-latest`. No `cross`, no QEMU.
...
2. **Targets**: darwin-arm64 (full), darwin-x64 (lexical-only, `--no-default-features`, cross-compiled on the arm64 runner and therefore not smoke-tested there), linux-x64 (glibc 2.35+), linux-arm64, windows-x64 (`mda.exe`). Musl is not built: ONNX Runtime has no musl binaries.
