...
## Done (last 5)
- Phase 2 search layer (ADR-0004): card embeddings (fastembed bge-small, static ONNX, schema v3), vector list fused with the two BM25 lists, `explain`, `timeline|recent|stale`, `rebuild --embeddings`, `embeddings`, MCP server `mda mcp` (7 tools, rmcp client test), `.mcp.json`, `mda eval` + golden set. Hybrid recall@5 0.983 / MRR 0.853 vs 0.883 / 0.747 lexical (`docs/benchmarks.md`).
...
## Next (max 3, in order)
1. Codex review of the Phase 2 step, triage, fixes; open the PR, CI, merge. Then confirm by hand that `.mcp.json` loads in Claude Code (`claude plugin validate .`, `/reload-plugins`).
