...
## Next (in order)
1. Benchmark plan (`docs/plans/2026-09-benchmarks.md`): **cards for the four DocsQA corpora are being produced** through the `claude-cli` backend (owner: free on the Max plan; `~/.cache/markdownattractor/bench/card-all.sh`, log `card-all.log`, ≈ 10 h, smallest corpus first); then export the cards to `evals/results/docsqa/cards-<version>-<project>.json` (export command still to write), then the carded and hybrid axis-A rows on the dev split (full coverage only: partial cards bias the fusion); the query-latency lever (≈ 0.5 s/question raw on the large corpora). Clones live under `~/.cache/markdownattractor/bench/`, the dataset under `bench/docsqa-data` (decompress `corpus.jsonl.gz`).
