...
## Next (max 3, in order)
1. Re-run CI on PR #10 (`gh run rerun <id>` or push), merge on a green job list, `gh workflow run release.yml --ref main`, read the job list, fix until all five builds and `publish` pass.
