...
## Done (last 5)
- **DocsQA adapter + ingestion gate**: `mda eval --dataset docsqa` (page-level success@5/MRR@5/nDCG@10, coverage report, seeded dev/test/holdout split, in `mda_core::eval`); all four projects at 100% label coverage; first axis-A row (raw lexical, dev split) published as the floor: success@5 0.31/0.22/0.33/0.60 (`docs/benchmarks.md`, `evals/results/docsqa/`).
...
## Next (in order)
1. Benchmark plan (`docs/plans/2026-09-benchmarks.md`): **cards for the four DocsQA corpora** through the `claude-cli` backend (§0a.3; ≈ 41K sections, bounded rounds, wall-clock published), committed under `evals/results/docsqa/`, then the carded and hybrid axis-A rows on the dev split; the query-latency lever (≈ 0.5 s/question raw on the large corpora). Clones live under `~/.cache/markdownattractor/bench/`, the dataset under `bench/docsqa-data` (decompress `corpus.jsonl.gz`).
