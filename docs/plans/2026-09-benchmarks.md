...
- [ ] B2 DocsQA: **ingestion gate passed** (100% label coverage on all four projects; evidence anchors 100/100/98/82%, the GitHub Docs gap is rendered Liquid includes, stated on the page; `docs/benchmarks.md`); first axis-A row (raw lexical, dev split) published as the floor; carding rate measured (≈ 1.1 sections/s, ≈ $175 list-price equivalent for all four corpora, which the owner confirmed is not a constraint on the Max plan; the full carding run was started on 2026-09-22 with `~/.cache/markdownattractor/bench/card-all.sh`). **Open** — cards for the four corpora through `claude-cli` (§0a.3), committed under `evals/results/docsqa/`; carded and hybrid rows at full coverage (partial cards bias the fusion: measured on Tailwind at 14%); qmd and BM25-over-files arms; axis B on the frozen test sample.
...
- [ ] Budget: no API-key spend at all (§0a.3). **No dollar budget either** (owner decision 2026-09-22): every call goes through the owner's Claude Code login on a Max plan, so the CLI's list-price equivalent is published for readers (what the same run would cost on the API) but is not a limit; the constraints are the plan's rate limits and wall-clock, both published under axis E. graphify's build cost recorded separately.
