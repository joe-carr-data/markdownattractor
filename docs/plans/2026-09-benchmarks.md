...
- [ ] B2 DocsQA: **ingestion gate passed** (100% label coverage on all four projects; evidence anchors 100/100/98/82%, the GitHub Docs gap is rendered Liquid includes, stated on the page; `docs/benchmarks.md`); first axis-A row (raw lexical, dev split) published as the floor; carding rate measured (≈ 1.1 sections/s, ≈ $175 list-price equivalent for all four corpora: **owner decision before it runs**). **Open** — cards for the four corpora through `claude-cli` (§0a.3), committed under `evals/results/docsqa/`; carded and hybrid rows at full coverage (partial cards bias the fusion: measured on Tailwind at 14%); qmd and BM25-over-files arms; axis B on the frozen test sample.
