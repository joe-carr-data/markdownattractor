...
**2026-09-25 amendment, before the T2 freeze (M6; the M5 pilot showed rule 0.8 read literally fails gate (c) for every arm).** The grounding check's claims are the *factual claims about the product* in an answer (a behaviour, option, default, version, API, selector, command, limit, quotation, or a statement that the documentation says something), each supported by a cited page verbatim or as a fair paraphrase; an answer's own reasoning that follows from supported facts, advice framed as a suggestion, and general programming knowledge the cited text does not contradict are not counted against it; a product fact the pages do not state or contradict, a quotation or warning they do not contain, and a citation to a page that does not contain the claim make it ungrounded. Citations resolve by one declared rule for every arm (an exact repository-relative path, or the unique suffix of one corpus page after a `<project>/` prefix); an unresolvable or oversized citation makes the answer ungrounded. The rubric text is hashed in `T2/FROZEN.md`. Rationale and pilot numbers: `docs/reviews/codex/2026-09-25-bench-m5.md`, `docs/benchmarks.md` § T2 pilot; decided by Claude on the owner's "go M6" after the recommendation to narrow (the rule's intent is fabrication, not explanation).
