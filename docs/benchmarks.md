...
- **graphify-haiku** (the model-matched configuration against mda's Haiku cards: Haiku 4.5 as the host of the whole build, fresh copies, no Sonnet cache) completed on all four projects in 340–454 s at $0.97–$6.14 list-price equivalent, but the Haiku host did not carry out graphify's own skill: it dispatched 6–49 extraction subagents where the skill's chunking called for 10–172, wrote graphs whose nodes are mostly unsourced (`_origin` absent on 73–99% of nodes), and covered 48 of 197, 44 of 770, 585 of 685 and 3,193 of 3,208 corpus pages with a node. Retrieval through `query_graph`: **0.040 / 0.000 / 0.000 / 0.061** success@5 (tailwind / supabase / prisma / github-docs). The comparison this configuration was meant to give (graph build cost at equal model) therefore does not exist yet: graphify's docs pipeline is an agent workflow whose fidelity depends on the host model, and at Haiku it is not the same tool. Both configurations stay on the page; the Sonnet one is the one to beat where it completed. Build records with per-model usage: `evals/results/docsqa/arms/graphify-*.json`.
