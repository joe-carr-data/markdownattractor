...
### Added
- **`mda eval --compare <label> <baseline results.json> <candidate results.json>`** (benchmark plan §3, 2026-09-24 amendment): a paired comparison of two archived runs, repeated per project: wins, losses, ties, the unrounded differences and descriptive 95% intervals from a within-project paired bootstrap (`--draws`, `--seed`; SplitMix64, so the draws are identical on every platform), with the objective (mean over projects) resampled jointly. `scripts/eval/tune.sh explore` runs the post-stop candidates against the unchanged baseline, information only, and refuses adoption.
- **Search tuning keys in `config.toml`** (benchmark M3, execution plan §3): `search_rrf_k` (60), `search_raw_weight` (1.0), `search_questions_weight` (2.0), `search_and_stopwords` (off) and `embedding_text` (`v1`, `questions-first`, `with-entities`; a variant changes the vector model id so the cards are re-embedded and never mixed). Defaults unchanged; `docs/design/search.md`. `scripts/eval/tune.sh` runs the greedy loop and writes `evals/results/docsqa/TUNING.md`.
