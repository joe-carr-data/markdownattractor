...
1. **L0 — search.** `mda_search(query="<the question, as the user would type it>", k=8)` or `"${CLAUDE_PLUGIN_ROOT}/scripts/mda" --json search "<question>" -k 8`. Add `since="7d"` (`--since 7d`) when the question is about recent changes, `path_prefix` (`--in <path>`) when the user named a folder. Hits are fused from cards, raw text and card vectors; `matched` and `vector` say which.
2. **L1 — read the card.** Each hit carries `tldr`, `heading_path`, `line_start`/`line_end`, `updated_at`, and `section_id`. For most questions the tldr plus one card (`mda_card(section_id)` / `mda --json card <section_id>`) is enough to answer, and it tells you how old the information is. Say the date when it matters.
