...
1. **L0 — search.** `"${CLAUDE_PLUGIN_ROOT}/scripts/mda" --json search "<the question, as the user would type it>" -k 8`. Add `--since 7d` when the question is about recent changes, `--status current` when it is about what applies now, `--in <path>` when the user named a folder.
2. **L1 — read the card.** Each hit carries `tldr`, `heading_path`, `line_start`/`line_end`, `updated_at`, and `section_id`. For most questions the tldr plus one card (`mda --json card <section_id>`) is enough to answer, and it tells you how old the information is. Say the date when it matters.
3. **L2 — open the exact lines.** `mda --json open <section_id>` returns the source lines for that section only (typically 20–60 lines). Quote from these, never from memory of the card. If the response has `stale: true`, the file changed after indexing; the lines you got are the current ones, but re-run the search if the answer looks off.
...
- retry the search with `--raw` (lexical only, over the full section text: catches config values, error strings, identifiers that a summary would omit);
